...
class HelperInfoPage extends StatelessWidget {
  ...
  Widget buildHelpersItem(int superIndex) {
    CreateAccountController _controller = Get.find();
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8.h),
          FutureBuilder(
              future: _controller.getHelperPermissions(),
              builder: (context, snapshot) => GridView.builder(
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  itemCount: _controller.listPermissions.length,
                  gridDelegate:
                  ...
                  itemBuilder: (context, index) {
                    PermissionData permission = _controller.listPermissions[index];
                    return Row(
                      children: [
                        GetBuilder<CreateAccountController>(
                            builder: (controller) => CustomCheckBox(
                                value: controller.getSelectedIndexes(superIndex, permission.id!),
                                checkedFillColor: AppHelper.getAppTheme(),
                                splashColor: AppHelper.getAppTheme(),
                                borderColor: Colors.white,
                                splashRadius: 9,
                                onChanged: (val) {
                                 controller.setSelectedIndexes(superIndex, permission.id!);

                                  print('FIRST PERMISSIONS: ${jsonEncode(controller.selectedIndexes1)}');
                                  print('SECOUND PERMISSIONS: ${jsonEncode(controller.selectedIndexes2)}');
                                  print('THIRD PERMISSIONS: ${jsonEncode(controller.selectedIndexes3)}');
                                  print('FOURTH PERMISSIONS: ${jsonEncode(controller.selectedIndexes4)}');
                                })),
                        AppText.medium(
                            text:
                            '${_controller.listPermissions[index].name}',
                            fontWeight: FontWeight.w400)
                      ],
                    );
                  }))
